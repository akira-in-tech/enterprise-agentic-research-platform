...
| Open-source contribution | Three open, one closed: [finos/FDC3#2091](https://github.com/finos/FDC3/pull/2091) is **open and ready for review with EasyCLA signed; its external Netlify preview initially failed, then passed after syncing the branch with upstream main** (resolves issue #1674; adds version-aware Workbench event listeners, event UI/examples, and seven tests). [kedro-org/kedro-plugins#1496](https://github.com/kedro-org/kedro-plugins/pull/1496) is **open and ready for review** and DCO-verified (adds `os.PathLike` Redis-key support plus save/load/exists regression coverage for the Redis slice of their issues #1316 and #1317). [HENNGE/terraform-aws-ecs#47](https://github.com/HENNGE/terraform-aws-ecs/pull/47) is **open** (closes their issue #43; documents the existing `triggers` + `plantimestamp()` workaround for forced ECS redeployment). [langchain-ai/langgraph#8583](https://github.com/langchain-ai/langgraph/pull/8583) was **auto-closed by their `require-issue-link` bot**, a process gate rather than a rejection of the content, and has not been resubmitted |
...
Submitted: a fourth real open-source contribution, [finos/FDC3#2091](https://github.com/finos/FDC3/pull/2091), directly resolving upstream issue #1674. The FDC3 Workbench now listens for `userChannelChanged` and Private Channel lifecycle/listener events through the FDC3 2.2 event API, keeps the displayed user-channel state synchronized, records the actual `details.contextType`, retains legacy callbacks for FDC3 2.0/2.1, and exposes event status/history plus copyable examples in the relevant tabs. Added seven focused Vitest cases; Workbench build/lint and the monorepo test/lint commands all passed. EasyCLA is signed and the PR is open and ready for maintainer review; the external Netlify deploy preview currently reports failure, and the PR is not described as merged.
Discovered on a later status check: finos/FDC3#2091's Netlify deploy preview was failing because the branch had fallen behind upstream main's website/build dependency baseline, not because of anything in the PR's own changes. Synced the branch with the latest main and pushed; the preview then reported pass (Deploy Preview ready). Caught by re-querying the PR's live check status rather than assuming the earlier submission-time snapshot still held, then corrected in the Component Status table and the charter's Open-Source Contribution section immediately. The PR remains open, not merged.
```
