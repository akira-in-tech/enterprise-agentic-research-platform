...
| Open-source contribution | One open, one closed: [HENNGE/terraform-aws-ecs#47](https://github.com/HENNGE/terraform-aws-ecs/pull/47) is **open** (closes their issue #43; explains that `force_new_deployment = true` alone doesn't force a redeploy on every apply and documents the existing `triggers` + `plantimestamp()` workaround, the same ECS force-redeploy mechanics this project verified directly via `scripts/aws-deploy.sh` and the restart verification). [langchain-ai/langgraph#8583](https://github.com/langchain-ai/langgraph/pull/8583) (a docs fix for a real `ssl`-vs-`sslmode` connection-string gap hit while wiring `AsyncPostgresSaver` into this project) was **auto-closed by their `require-issue-link` bot** — a process gate (external PRs need a pre-approved linked issue), not a rejection of the content — and has not been resubmitted |
