...
| Open-source contribution | Three open, one closed: [finos/FDC3#2091](https://github.com/finos/FDC3/pull/2091) is **open as a draft with EasyCLA pending and its external Netlify preview failing** (resolves issue #1674; adds version-aware Workbench event listeners, event UI/examples, and seven tests). [kedro-org/kedro-plugins#1496](https://github.com/kedro-org/kedro-plugins/pull/1496) is **open and ready for review** and DCO-verified (adds `os.PathLike` Redis-key support plus save/load/exists regression coverage for the Redis slice of their issues #1316 and #1317). [HENNGE/terraform-aws-ecs#47](https://github.com/HENNGE/terraform-aws-ecs/pull/47) is **open** (closes their issue #43; documents the existing `triggers` + `plantimestamp()` workaround for forced ECS redeployment). [langchain-ai/langgraph#8583](https://github.com/langchain-ai/langgraph/pull/8583) was **auto-closed by their `require-issue-link` bot** — a process gate, not a rejection of the content — and has not been resubmitted |
