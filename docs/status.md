...
| Open-source contribution | Two open, one closed: [kedro-org/kedro-plugins#1496](https://github.com/kedro-org/kedro-plugins/pull/1496) is **open as a draft** and DCO-verified (adds `os.PathLike` Redis-key support plus save/load/exists regression coverage for the Redis slice of their issues #1316 and #1317). [HENNGE/terraform-aws-ecs#47](https://github.com/HENNGE/terraform-aws-ecs/pull/47) is **open** (closes their issue #43; documents the existing `triggers` + `plantimestamp()` workaround for forced ECS redeployment). [langchain-ai/langgraph#8583](https://github.com/langchain-ai/langgraph/pull/8583) was **auto-closed by their `require-issue-link` bot** — a process gate, not a rejection of the content — and has not been resubmitted |
