...
| Open-source contribution | Two open PRs, neither merged yet: [langchain-ai/langgraph#8583](https://github.com/langchain-ai/langgraph/pull/8583) (a docs fix for a real `ssl`-vs-`sslmode` connection-string gap hit while wiring `AsyncPostgresSaver` into this project) and [HENNGE/terraform-aws-ecs#47](https://github.com/HENNGE/terraform-aws-ecs/pull/47) (closes their issue #43; explains that `force_new_deployment = true` alone doesn't force a redeploy on every apply and documents the existing `triggers` + `plantimestamp()` workaround, the same ECS force-redeploy mechanics this project verified directly via `scripts/aws-deploy.sh` and the restart verification) |
