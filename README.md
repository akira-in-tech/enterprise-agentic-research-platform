...
| Evaluation harness | scripts/run_evaluation.py scores routing accuracy, source coverage, private-knowledge accuracy, report-section coverage, completion rate, human-review trigger rate, and latency against evaluation_cases.jsonl; scoring/loading unit tested against the real fixture file and a mocked HTTP transport. Four real runs published (2026-08-07, `--provider qwen`, local Ollama, no paid provider) across three rounds of root-caused, verified fixes (evidence-pool noise capping, an `ORIGIN` field/instruction for preferring private sources, then enabling Qwen3 thinking mode): source coverage improved 80%→100% at one point, and thinking mode fixed a genuine catastrophic-topic-drift failure mode (a Redis-security report that devolved into dictionary definitions), but overall pass rate held at 20% across all four runs — report-section coverage stayed near zero because the model writes topically sound but differently-worded headings than the fixture expects (a scoring-method blind spot, not purely a model one), and private-knowledge citation never reliably worked despite the private source consistently ranking #1 by score with an explicit origin label and instruction. Real, published, unflattering findings, not swept under the rug — see [docs/evaluation.md](docs/evaluation.md#fourth-run-enabling-qwen3-thinking-mode) |
