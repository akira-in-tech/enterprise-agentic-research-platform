...
| Evaluation harness | scripts/run_evaluation.py scores routing accuracy, source coverage, private-knowledge accuracy, report-section coverage, completion rate, human-review trigger rate, and latency against evaluation_cases.jsonl; scoring/loading unit tested against the real fixture file and a mocked HTTP transport. Three real runs published (2026-08-07, `--provider qwen`, local Ollama, no paid provider), the last two after root-causing and fixing evidence-pool noise (a top-scored-sources cap) and a missing `ORIGIN` field/instruction for preferring private sources: source coverage improved 80%→100% and held, but report-section coverage and private-knowledge accuracy did not reliably improve despite two targeted, verified fix attempts — root-caused to a real capability ceiling of the local 8B model (the private source ranked #1 by score with an explicit origin label and instruction, and was still ignored), not a retrieval, scoring, or prompt-completeness bug. Overall pass rate stayed at 20% across all three runs. Real, published, unflattering findings, not swept under the rug — see [docs/evaluation.md](docs/evaluation.md#second-and-third-runs-after-two-rounds-of-real-fixes) |
