...
| Evaluation harness | scripts/run_evaluation.py scores routing accuracy, source coverage, private-knowledge accuracy, report-section coverage, completion rate, human-review trigger rate, and latency against evaluation_cases.jsonl; scoring/loading unit tested against the real fixture file and a mocked HTTP transport. Five real runs published (2026-08-07): four with `--provider qwen` (local Ollama, no paid provider) across three rounds of root-caused, verified fixes (evidence-pool noise capping, an `ORIGIN` field/instruction for preferring private sources, then enabling Qwen3 thinking mode — which fixed a catastrophic-topic-drift failure mode but left overall pass rate at 20%), then one `--provider claude` run (first real-cost call, explicit go-ahead) for direct comparison. Claude cleanly followed the private-source-preference instruction qwen3:8b failed four times in a row (private-knowledge accuracy 0%→100%) and ran roughly twice as fast (283s→116s average), but landed on the same 20% overall pass rate for different reasons: report-section coverage stayed low for both providers alike (confirming that metric under-credits topically sound, differently-worded headings — a scoring-method issue, not only a model one), and the Claude run surfaced a real crash bug — `AnalystAgent.analyze()` had no fallback for a truncated structured response, unlike `EvidenceJudgeAgent`, and an unhandled 500 resulted; fixed (raised token budget, added the same fallback pattern) and unit tested in the same commit as the write-up. Real, published, unflattering findings, not swept under the rug — see [docs/evaluation.md](docs/evaluation.md#fifth-run-provider-claude-comparison-first-real-cost-run) |
