...
def test_normalize_url_removes_tracking_and_fragment() -> None:
    result = normalize_url(
        "HTTPS://Example.COM:443/docs/?utm_source=newsletter&b=2&a=1#installation"
    )
...
def test_normalize_url_preserves_non_default_port() -> None:
    result = normalize_url("http://Example.COM:8080/api/?page=2")

    assert result == ("http://example.com:8080/api?page=2")
