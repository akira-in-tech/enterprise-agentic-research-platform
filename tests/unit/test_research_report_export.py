...
async def test_export_and_retrieve_round_trip(tmp_path: Path) -> None:
    ...
    assert key == (
        f"tenants/{tenant_id}/report-exports/{research_run_id}/report-numbered.md"
    )
    assert (tmp_path / key).exists()

    retrieved = await service.retrieve(tenant_id=tenant_id, research_run_id=research_run_id)

    assert retrieved == b"# Report\n\nHTTP/3 evidence. [1]"


@pytest.mark.anyio
async def test_export_and_retrieve_respect_format_and_citation_style(tmp_path: Path) -> None:
    storage = LocalDocumentStorage(tmp_path)
    service = ResearchReportExportService(storage)
    tenant_id = uuid4()
    research_run_id = uuid4()
    ...
    assert key == (
        f"tenants/{tenant_id}/report-exports/{research_run_id}/report-footnote.pdf"
    )
    ...
    assert retrieved == b"%PDF-1.7 fake pdf bytes"
